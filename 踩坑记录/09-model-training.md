...
训练完成的最小交付是：可读取的数据、可重载的 checkpoint、完整日志和可校验归档。它不是“策略成功”的证据。选择 checkpoint 时至少应固定任务文本、相机配置、初始位、最大运行时间和成功判据，并保留每条运行视频。

本项目后续已经完成了绿色与紫色 2k、10k、20k 的各 10 条真实运行。结果显示 10k 不一定等于最后 checkpoint 之外的唯一候选，且绿色与紫色的表现显著不同；完整统计见 [真实评估总报告](../项目流程/09-real-world-evaluation.md)。因此后续训练不应默认“步数越大越好”，应让真实、独立复位的评估决定 checkpoint 选择。
